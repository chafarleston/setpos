...
public class JPanelCloseMoney extends JPanel implements JPanelView, BeanFactoryApp {
   ...
   //Email -- Corvisier
   private void initEmailParameters() {
        sMailHost = appProp.getProperty("mail.host");
        sMailPort = appProp.getProperty("mail.port");
        sMailEmail = appProp.getProperty("mail.emailaddress");
        sMailEmailTo = appProp.getProperty("mail.sendto");
        sMailPassword = appProp.getProperty("mail.password");
        sMailTLS = appProp.getProperty("mail.tls");
        sMailAuth = appProp.getProperty("mail.auth");

        if (sMailEmail != null && sMailPassword != null && sMailPassword.startsWith("crypt:")) {
                AltEncrypter cypher = new AltEncrypter("cypherkey" + sMailEmail);
    ...
    private void m_jCloseCashActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jCloseCashActionPerformed
        ...
        if (res == JOptionPane.YES_OPTION) {
            ...
            props.setProperty("mail.smtp.host", sMailHost);
            props.setProperty("mail.smtp.starttls.enable", sMailTLS);
            props.setProperty("mail.smtp.port", sMailPort);
            props.setProperty("mail.smtp.auth", sMailAuth);
            props.setProperty("mail.smtp.user", sMailEmail);
            ...
            message.setFrom(new InternetAddress(sMailEmail));
            message.addRecipient(
                Message.RecipientType.TO,
                new InternetAddress(sMailEmailTo));
            message.setSubject("Cash closed performed on: " + m_App.getProperties().getHost());
            message.setText("openPOS closed cash on: " + dNow + " with a total of: " + m_PaymentsToClose.printSalesTotal());

            // Send message
            Transport t = session.getTransport("smtp");
            t.connect(sMailHost, sMailEmail, sMailPassword);
